...
Na home, os dois cards pré-REN do topo usam o agregado histórico de `kpi_overview`. Já os cards pós-REN e as variações do topo são uma visão Brasil fixa recalculada no cliente a partir de `serie_mensal_nacional`, usando a janela operacional 2023–2025; os filtros de empresas afetam apenas os gráficos e cards inferiores. Os gráficos inferiores de classe/localidade usam `/api/v2/home-service-types`, que lê `fato_transgressao_mensal_porte` via PostgreSQL quando disponível e cai para o CSV analítico exato quando o banco está degradado. Essa rota preserva zeros reais e as quatro categorias `urbana`, `rural`, `grupo_a` e `nao_classificado`; por isso 2025/2026 e os filtros devem bater com os totais anuais do escopo selecionado.
