...
> Na home, os cards pré-REN do topo permanecem no agregado histórico `kpi_overview`; os cards pós-REN e deltas do topo são calculados como visão Brasil fixa sobre `serie_mensal_nacional` na janela operacional 2023–2025. Os filtros de empresas continuam restritos aos gráficos e cards inferiores. Esses gráficos inferiores usam `/api/v2/home-service-types`, priorizando PostgreSQL (`fato_transgressao_mensal_porte`) e caindo para o CSV analítico exato; a UI deve preservar zeros reais e exibir as quatro categorias `urbana`, `rural`, `grupo_a` e `nao_classificado` para 2025/2026 e demais filtros.
