...
- Porta `8050`: conflito resolvido ao parar container Docker legado (`tcc-ren1000-app`)
...
5. Validar endpoints
   ...
   - `curl -s http://localhost:8050/api/dashboard | jq '.meta.generated_at'`
6. Validar UI com Playwright (se necessario)
   - `export PWCLI="$HOME/.codex/skills/playwright/scripts/playwright_cli.sh"`
   - `"$PWCLI" --config .playwright/cli.config.json open http://localhost:8050`
